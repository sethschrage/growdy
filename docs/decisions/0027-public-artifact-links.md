...
The rewrite outlived it, and has now gone too. The Decision above required a `vercel.json` rewrite (`/(.*) -> /index.html`) because Vercel's static file server 404s on any path but `/`; `#241` removed the route and left the rewrite behind. Deleted 2026-09-21: `app/src/` holds no `pathname` read, no history call and no router, the manifest's `start_url` and `scope` are both `/`, and nothing in the client asks Supabase to redirect anywhere else -- so `/` is the only path anything resolves, and a catch-all answering every other URL with a 200 and the app shell was left asserting a routing story the app no longer has. `app/vercel.json` contained nothing else, so the file went with it. A later web client that wants routing adds it back, and this paragraph is the reason it was ever there.
