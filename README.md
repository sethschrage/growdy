...
with two narrow exceptions recorded in `0038`'s 2026-09-22 update
(keeping shared data correct, and review features that fit the desktop),
until it is rebuilt later as a second thin client against the same API.

Everything that is not a user interface is shared by both clients: the
Postgres schema and its RLS, the six Edge Functions, the auth model, the
prompts. The one planned change is conversation saving, which moves into
a database function both clients call
([`0039`](docs/decisions/0039-how-the-native-client-is-built-tested-and-delivered.md));
it gets its own ADR. What carries between the two clients is the API
...
- [0038 -- The phone gets its own client](docs/decisions/0038-the-phone-gets-its-own-client.md) -- the iOS app is rebuilt in SwiftUI and becomes the full-featured client; the React app is frozen as a desktop surface and rebuilt later against the same API. Decided on measurement: 436 lines of client code exist only to fight `WKWebView` keyboard and gesture behaviour, six of `0.15.0`'s forty-two commits were touch fixes, and a device probe proved `backdrop-filter` silently ignores an SVG filter, so the intended glass is unreachable in CSS at any price. The schema, the Edge Functions and the auth model are shared and untouched. **Amended 2026-09-22**: the freeze has two narrow exceptions -- shared-data correctness and desktop review features -- and conversation saving moves to the server ([0039](docs/decisions/0039-how-the-native-client-is-built-tested-and-delivered.md)).
- [0039 -- How the native client is built, tested and delivered](docs/decisions/0039-how-the-native-client-is-built-tested-and-delivered.md) -- `native/` at the root, logic in a local Swift package tested on the Mac in seconds and screens in a thin app target, MapLibre confined to the map screens; iOS 26.0 and up, screens checked on 26.5 and 27 simulators; a free Apple team until a second producer needs the app, installed over Wi-Fi and re-signed weekly, never deleted to fix an expiry. Settled one decision at a time after an audit of the build prompt found it would have shipped silent bugs, and it lists every place the native client deliberately does not copy the web one.
...
| Tests | Vitest + Testing Library, in `app/` beside what they test -- run in CI on every PR alongside `tsc` and `oxlint`; Deno's test runner for `supabase/functions/`, in the `functions` job. The native client's will be Swift Testing (`0039`) |
| Client, primary | A native SwiftUI iOS app, which becomes the full-featured one (see [`docs/decisions/0038`](docs/decisions/0038-the-phone-gets-its-own-client.md)). **Not written yet.** How it is built, tested and delivered is decided in [`0039`](docs/decisions/0039-how-the-native-client-is-built-tested-and-delivered.md): `native/`, an app target over a local Swift package tested with Swift Testing on the Mac, iOS 26.0 and up, installed over Wi-Fi on a free Apple team |
| Client, frozen | React (Vite), in `app/` -- organised by feature over a shared data layer (see [`docs/decisions/0032`](docs/decisions/0032-client-organised-by-feature.md)), and [`0008`](docs/decisions/0008-app-as-research-tool.md) for why it exists at all. Frozen by `0038` as a desktop surface: still deployed, still maintained, no new features -- except where shared data would otherwise go wrong, and review features that fit a desktop (`0038`'s update) |
| iOS, today | Capacitor shell wrapping the React build, in `app/ios/` -- plugins: camera, geolocation, social login, keyboard. Built with `xcodebuild` and installed on the producer's phone over Wi-Fi, re-signed weekly on a free Apple team, and what each release is exercised on; no App Store listing, and Sign in with Apple is required before there can be one (see [`docs/decisions/0029`](docs/decisions/0029-ios-shell-and-native-sign-in.md)) |
| Hosting | [Vercel](https://app-blue-ten-25.vercel.app), connected to this GitHub repo -- auto-deploys production from `main` and a preview per branch/PR, but only for pushes that change something under `app/` (`app/vercel.json`'s `ignoreCommand` skips the rest) |
